...
- Add a `task_runner` question (`Makefile`/`Justfile`, default `Makefile`) so generated projects can choose their dev-command runner; the full_project Justfile mirrors the Makefile's Docker/Django/database/quality/document-first targets (plus the `use_vite` frontend recipes when enabled), using `just`'s native doc-comments and `just --list` instead of `make help`'s hand-rolled parser. (reusable_app Justfile support to follow)
